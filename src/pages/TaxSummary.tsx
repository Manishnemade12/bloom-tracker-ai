...
const TaxSummary = () => {
  ...
  useEffect(() => {
    if (user) {
      supabase.from("financial_data").select("*").eq("financial_year", "2025-26").single().then(({ data }) => setFinancialData(data));
      supabase.from("tax_analyses").select("*").eq("financial_year", "2025-26").single().then(({ data }) => setAnalysis(data));
    }
  }, [user]);
